$(function(){
	...
	$("#add").bind("click", function(){
		addUpdDialog({
			htmlPath: path+'/base/addDepartPage.do', 
			title: "新增部门", 
			saveFun: function(){
				if(!($("#menuAddUdpForm").myValidate())){
					return false;
				}
				myAjaxReqJson(path+'/base/saveDepartment.do', function(result, textStatus){
					myDialog("保存成功！", 1);
	...
	$("#del").bind("click", function(){
		...
		if (!selectedRowId || selectedRowId.length==0) {
			...
			myDialogConfirm("您确定要删除选中的记录嘛？", function(){
				myAjaxReqJson(path+'/base/delDepartment.do', function(result, textStatus){
					myDialog("删除成功！", 1);
					$("#grid").trigger("reloadGrid");
	...
	$.fn.myDataGrid({
		// 列表数据请求路径
		gridUrl: path+'/base/getDepartList.ajax',         
		// 表格渲染table ID
		gridId: 'grid',
		// 分页渲染DIV ID
		pagerId: 'page',
		// 表头
		colNames: ['ID', '上级部门', '部门名称', '部门描述', '显示顺序', '操作'],
		colModel: [{								
			name:'ID',
			index:'ID',
			sortable:false,
